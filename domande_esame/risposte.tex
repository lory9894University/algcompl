...
%TODO: premettere cosa fa distribute e concat_map (e anche permutation, ma è esplicativo). 
...
\end{figure*}\\
	...
	Sostituendo $len(t)$ a $len(l')$ nell'equazione che specificava il numero di elementi (TODO: fare un ref all'equazione) delle liste otteniamo che $permutation  \; h::t$ genera una lista di liste di $(1+len(t)) * len(t)!$ elementi.\\ %Todo: leggi
...
\end{itemize}
%TODO: per entrambi mostrare il cammino che porta alla risposta
\section{Backtrack}
\subsection{B.1}
...
\end{figure*}\\
	...
	\end{enumerate}
\end{itemize}
%TODO:  INSERIRE UN ESEMPIO PER OGNUNA (grafo, matrice e albero di esplorazione). si possono prendere quelli del prof
\subsection{B.3}
...
%TODO: ruben ha aggiunto "mancanza di una euristica per sapere quando andare in profondità" (ad esempio nella foto a sinistra di figura 12, quando siamo al nodo 30 basterebbe scendere di un nodo)
...
%TODO: qui serve l'immagine del prof di tutti gli alberi della ricorsione, magari uno snippet
...
\begin{definition}[Riduzione]
	...
	\end{itemize}
\end{definition}
%todo: aggiungere 2 proprietà: transitività e "se risolvo p1 in Ptime allora risolvo P2 in pt" la complessità temporale rimane
...
%todo: aggiungere il discorso di ruben, basata sulle proprietà, screenshot. no, non lo screen semplicemente dimostrare basandosi sulle proprietà.
%todo: 7.5.1 Perché la riduzione è per noi rilevante?
\begin{itemize}
...
\end{figure*}\\
%todo: mancano grossi pezzi. 2 lemmi vengono mantenuti. Vedere screenshot e dispense.
%ruben per ogni lemma propone di fare un esempio, l'aspetto fondamentale sono i 2 lemmi! (1 caso per il lemma 1 ed il lemma 2)
\subsection{E.7}
...
%TODO: in questo problema in realtà non credo sia possibile avere una penalità < dell'energia, solo uguale.
\begin{figure*}[!ht]
\centering
...
\end{enumerate}
%TODO: Esempio? prendo quello del prof? se lo metto lo riscrivo in latex
\subsection{F.6}
...
%TODO: l'esempio del prof con il >= sembra errato.
...
Lo scopo del lagrangiano è far sì che la penalità introdotta da una scelta che viola un vincolo sia così sconveniente da non poter essere "bilanciata" da alcun valore di $\mathcal{H}_obb$, per quanto basso questo possa essere.
...
%TODO: inserire un problema di esempio
\end{document}
