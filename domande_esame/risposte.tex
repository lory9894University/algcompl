...
\end{figure*}\\
	...
	Sostituendo $len(t)$ a $len(l')$ nell'equazione che specificava il numero di elementi ($(1 + len(l')) * len(t)! $) delle liste otteniamo che $permutation  \; h::t$ genera una lista di liste di $(1+len(t)) * len(t)!$ elementi.\\ %Todo: leggi
...
\end{dimostrazione}
%TODO:ruben per ogni lemma propone di fare un esempio, l'aspetto fondamentale sono i 2 lemmi! (1 caso per il lemma 1 ed il lemma 2)
\subsection{E.7}
...
\end{align*}
In tal modo è impossibile assegnare due colori allo stesso nodo (avremmo una ripetizione della variabile) ed è anche impossibile assegnare a due nodi adiacenti lo stesso colore (avremmo una intersezione di due insiemi o una non copertura).%TODO:se prendo x prendo anche v_x o r_x o b_x (fingiamo che io prenda v_x). in questo modo sto lasciando fuori v_xy che si deve legare ad un' altro ?_yx che quindi non potrà più stare in un una tripletta. non può stare con v_yx? eh no, sarei costretto a prendere v_y e quindi y dato che non posso prendere la tripletta v_xy, v_y, v_yz
...
%argomentare meglio il discorso di poliniomio. video di ciò che ha scritto a Ruben.
